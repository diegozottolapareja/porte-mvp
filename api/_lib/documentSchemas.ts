...
const DATA_SCHEMA = {
  ...
  properties: {
    ...
    costoMat: { type: ['number', 'null'], description: 'Solo para budget, y solo si el documento desglosa explícitamente el costo de materiales como rubro interno — no lo confundas con el precio de un ítem de la cotización. Dejar null si el documento es una cotización simple (ítems + Sub Total + IVA + Total).' },
    costoMo: { type: ['number', 'null'], description: 'Solo para budget, mismo criterio que costoMat: solo si el documento desglosa explícitamente el costo de mano de obra como rubro interno.' },
    indVendidos: { type: ['number', 'null'] },
    impuestos: { type: ['number', 'null'], description: 'Para expense/income, el impuesto del comprobante. Para budget, el monto de IVA de la cotización (ej. "IVA 21%").' },
    comercial: { type: ['number', 'null'] },
    beneficio: { type: ['number', 'null'] },
    subtotal: { type: ['number', 'null'], description: 'Subtotal antes de impuestos. Para budget, el "Sub Total" de la cotización (antes del IVA).' },
    monto: { type: ['number', 'null'], description: 'Monto/total final del documento. Para expense/income es el total del comprobante; para budget es el "Total" final de la cotización (con IVA incluido).' },
